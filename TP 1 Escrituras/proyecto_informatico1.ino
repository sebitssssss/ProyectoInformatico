...
int leerPorcentaje() {
  int valorBruto = analogRead(PIN_LDR);
  int porcentaje = map(valorBruto, 0, 1023, 0, 100);
  porcentaje = constrain(porcentaje, 0, 100);
  return porcentaje;
}

int porcentajeALeds(int porcentaje) {
  int leds;
  if (porcentaje <= 30) {
...
String nombreColorParaLed(int indice) {
  if (indice <= 1) {
    return "VERDE";
  } else if (indice <= 3) {
    return "AMARILLO";
...
void actualizarLeds(int cantidadLeds) {
...
void actualizarLCD(int porcentaje, int cantidadLeds) {
  lcd.setCursor(0, 0);
  lcd.print("                ");
  lcd.setCursor(0, 0);
...
void revisarBoton() {
  bool estadoActual = digitalRead(PIN_BUTTON);
